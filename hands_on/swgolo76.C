...
//  6. Compile with:                                                                         //
//     > g++ -g swgolo72.C `root-config --libs --cflags` -o swgolo72                         //
//     (or whatever version of the code we have gotten to)
//     or if using derivgrind:                                                               //  
//     > g++ -g swgolo72.C -D WITH_DERIVGRIND `root-config --libs --cflags` -o swgolo72      //
//     In that case first also do                                                            //
//     > source Export_derivgrind.sh                                                         //
//     (sets up paths)                                                                       //
//     and run it with                                                                       //
//     > valgrind --tool=derivgrind ./swgolo72                                               //
//     (step 7)                                                                              //
//  7. Run it by specifying all parameters you wish to test. E.g.:                           //
//     > nohup ./swgolo72 -nev 2000 -nba 2000 -nde 100 -nep 500 -sha 101 -nth 1  &           //
//                                                                                           //
...
//  --------------------------------------------------                                       //
//  - Implement choice of initial layouts as per SWGO baselines - done                       //
...
//  - Fix indfile issue when readgeom is true (adding epochs) - done?                        //
//                                                                                           //
//  Updates:                                                                                 //
//  -------------------------------------------------------------------------------------    //
...
//  v65  Small fixes to commonmode 1 LR, Rdistr0 plot, renaming a few vars for consistency;  //
//       added CommonMode=3 for symmetrical movements (triangular symmetry - use w/ shape=4  //
//  v66  Renamed spanR into maxRdet and added minRdet as we will generate showers in annulus //
//  v67  Use trials/successes in shower generation to only illuminate detectors within a     //
//       radius spanned by maxRdet+Rslack, correcting for density the utility. This for now  //
//       only works if FixShowerPos is off and if CircleShowers is on                        //
//  v68  Correct calculation of average r displacement if CommonMode=3; fix minus sign in    //
//       calculation of energy derivative for U_IR                                           //
//  v69  interim tries to add density factor to U_GF                                         //
//  v70  complete calculation of U_GF with density factor; added N_active, removed minRdet   //
//  v71  Generalize to multiplets the symmetrical runs with CommonMode == multiplicity,      //
//       and other fixes of utility and its derivatives                                      //
//  v72  Changed maxRdet into aveRdet and modified derivative of Utility accordingly         //
//  v73  Various small fixes. Option of not performing SGD, and of using true shower pars    //
//  v74  TankNumber now a parameter to ease specification of macroaggregates                 //
//  v75  Small fixes (root output name, outfile U values). Options added(-itv, -nsu)         //
///////////////////////////////////////////////////////////////////////////////////////////////
...
static bool         doSGDupdate    = true;     // Set to false if not changing detector layout
...
static bool         fixEto1        = false;     // If on, all showers are generated at 1 PeV
...
static const int    maxIter        = 10;        // Number of recorded trials in shower generation (for dU/dx calc.)

// Global parameters required for dimensioning arrays, and defaults
// ----------------------------------------------------------------
static int Nunits                  = 300;       // Number of detector units
...
static int Nepochs                 = 500;       // Number of SGD cycles performed
...
// -----------------------------
static double aveRdet              = 0.;        // Span in R of detector array. This gets defined based on the initial layout of x[], y[]
static int indmaxR                 = Nunits-1;  // index of detector at max radius
...
static double TankArea             = 68.59*pi;  // 68.59 pi for 19 hexagonal shape macro units // 25.27pi for 7 1.9m radius "macro-tanks" aggregates in hexagonal pattern (works better than smaller units)
...
static const double delta2         = 0.04;      // Regularization factor in integrated resolution calculation
static double Wslope               = 1.;        // Slope with logE of weight of event for resolution contribution to utility.
...
static double Rslack               = 2000.;     // This parameter controls how far away to generate showers around array area
...
static double U_IR_Num;                         // Numerator in integrated resolution contribution to utility
static double U_IR_Den;                         // Denominator in integrated resolution contribution to utility
static double Rslack2;                          
static double dRtot_dx;
static double dRtot_dy;
static double U_GF;
static double U_IR;
...
static int totNtrials;
static double TryX0[maxEvents][maxIter];
static double TryY0[maxEvents][maxIter];
...
static TProfile * DE    = new TProfile ("DE",    "E resolution vs true E, running config", 11, 0.1, 10., 0., 1000.); 
static TProfile * DE0   = new TProfile ("DE0",   "E resolution vs true E, initial config", 11, 0.1, 10., 0., 1000.); 
#ifdef FEWPLOTS
...
static TH2D *     PGvsD  = new TH2D     ("PGvsD",  "G probability of triggering vs D and E", 25, 0., 2500., 10, 0., 10.);
...
void DefineLayout () {
    ...
    if (shape==0) { // hexagonal grid
        ...
        // If CommonMode>=3, we repeat the pattern CommonMode times at 360/mult degree spacing
        // -----------------------------------------------------------------------------------
        if (CommonMode>=3) {
        ...
        for (int id=0; id<Nunits; id++) {
            x[id] = XDoffset -DetectorSpacing/2.+DetectorSpacing*(id%2);
            y[id] = YDoffset -Nunits*0.25*DetectorSpacing+DetectorSpacing*(id/2);
        }
...
void SetShowersXY() {
    if (setXYto00) {
        ...
        double r0 = sqrt(pow(aveRdet+Rslack,2)*pi/Nevents);
        TrueX0[0] = Xoffset;
        TrueY0[0] = Yoffset;
        int is   = 1;
        int n    = 6;
        ...
        for (int is=0; is<Nevents; is++) {
            TrueX0[is] = Xoffset -aveRdet -Rslack + 2.*(aveRdet+Rslack)*(is%side+0.5)/side;
            TrueY0[is] = Yoffset -aveRdet -Rslack + 2.*(aveRdet+Rslack)*(is/side+0.5)/side;
        ...
        side = sqrt(Nbatch);
        for (int is=Nevents; is<Nevents+Nbatch; is++) {
            TrueX0[is] = Xoffset -aveRdet -Rslack + 2.*(aveRdet+Rslack)*((is-Nevents)%side+0.5)/side;
            TrueY0[is] = Yoffset -aveRdet -Rslack + 2.*(aveRdet+Rslack)*((is-Nevents)/side+0.5)/side;
            if (debug) cout << is << " x,y= " << TrueX0[is] << " " << TrueY0[is] << endl;
        }
    }
}
...
void GenerateShower (int is) {
        ...
        if (SameShowers && is>=Nevents) {
            ...
            do {
                if (CircleExposure) {
                    ...
                    // -------------------------------
                    double DfromCenter = sqrt(myRNG->Uniform(0., pow(aveRdet+Rslack,2)));
                    ...
                    // ------------------------------------
                    tryx0 = Xoffset -(aveRdet+Rslack)+myRNG->Uniform(2.*(aveRdet+Rslack));
                    tryy0 = Yoffset -(aveRdet+Rslack)+myRNG->Uniform(2.*(aveRdet+Rslack));
                }
                double r2;
                for (int id=0; id<Nunits && out; id++) {
                    r2 = pow(x[id]-tryx0,2)+pow(y[id]-tryy0,2);
                    if (r2<Rslack2) out = false;
                }
                // Store trials for dudx,dy calculation
                // ------------------------------------
                iter = Ntrials[is];
                if (iter<maxIter && out) {                         
                    TryX0[is][iter] = tryx0;
    ...
    if (SameShowers) {
        if (scanU || fixEto1) {
            ...
            if (is<Nevents) {
                ...
                if (Eslope!=0.) {
                    TrueE[is] = pow(pow(Emin,-Eslope)+(is-Nevents+0.5)/Nbatch*(pow(Emax,-Eslope)-pow(Emin,-Eslope)),-1./Eslope);
                } else {
    ...
    // ----------------------------------------
    GetCounts (is); // This also fills SumProbgt1[]
    return;
}
...
double FitShowerParams (int is, bool GammaHyp) {
    ...
    if (!initTrueVals || FalseHypothesis) {
        ...
        currentTheta = 0.5;
        currentPhi   = 0.;
        currentE     = 1.; // Just a first guess, midpoint of log_10 E between 100 TeV and 10 PeV
        ...
        if (initBitmap<8 && !usetrueXY) { // then we do not initialize x0,y0 to true values.
            ...
            for (int ie=0; ie<N_e; ie++) {
                ...
                for (int ix=0; ix<N_g; ix++) {
                    if (N_g>1) cX0 = (-1.+(1.+2.*ix)/N_g)*(aveRdet+Rslack);
                    for (int iy=0; iy<N_g; iy++) {
                        if (N_g>1) cY0 = (-1.+(1.+2.*iy)/N_g)*(aveRdet+Rslack);
                        // Compute likelihood for this point
...
double ComputePDF (int k, bool gammahyp) {
    ...
    // --------------------------------------------------------------------------------------------- 
    if (Norm>0.) p /= Norm; 
    if (p<epsilon2) p = epsilon2; // Protect against outliers screwing up calculations
    return p;
}
...
double dEk_dRik (int id, int is) {
    ...
    do {
        ...
        olddist = dist;
        iloop++;
    } while (fabs(dist)>fabs(distmax) && iloop<maxloops); // && increment<4.*aveRdet);
...
void ComputeUtilityGF () {
    ...
    U_GF = eta_GF * MeasFg / MeasFgErr * ExposureFactor; // scale with inverse sqrt(density of showers)
    cout    << " U_GF = " << U_GF;
    outfile << " U_GF = " << U_GF;
    return;
}
...
void threadFunction (int threadId) {
    ...
    int ismin = threadId*Nperthread;
    int ismax = (threadId+1)*Nperthread;
...
void threadFunction2 (int threadId) { 
    ...
    int idmin = threadId*Nperthread;
    int idmax = (threadId+1)*Nperthread;
    ...
    for (int id=idmin; id<idmax; id++) { 
        ...
        for (int k=Nevents; k<Nevents+Nbatch; k++) {
            ...
            // ----------------------------------------------------------------------------
            sumdPAkdxi_noweight += ProbTrigger (SumProbgt1[k],1,id,k); // derivative wrt dx
            sumdPAkdyi_noweight += ProbTrigger (SumProbgt1[k],2,id,k); // derivative wrt dy
            if (!Active[k]) continue;
            ...
            if (eta_IR!=0.) {
                ...
                //           den  = sum_k {PA[k]*Wk}
                //           num' = sum_k {dPA[k]/dxi *Wk*deltaEk + (NB)
                //                  2PA[k]*Wk*Et^2*(Et-Ek)/[Et^2*delta2+(Et-Ek)^2]^2) * dEk/dRik * dRik/dxi } 
                //                  (note: minus sign from der. of Ek, times minus sign as it is at denominator)
                ...
                if (IsGamma[k]) {
                    ...
                    if (fabs(SumProbgt1[k]-Ntrigger)<0.1*Nunits) { // was <1
                    ...
                    double factor = 2. * PActive[k] * Wk * Et2 * dEk / pow(deltaEk,2) * dEdR;
        ...
        //    sqrt(1/rho) = EF, 
        // ie. 
        //    EF = sqrt(Rtot^2*Nbatch)/(Ntrials*Nactive), 
        ...
        //    drhoinv/dx = rhoinv*( 1/Rtot dRtot/dx + 1/Ntrials * dNtrials/dx - 1/Nactive * dNactive/dx).
        // dRtot/dx can be obtained by checking how R varies only for the detector at max r, indmaxR. (this is 
        ...
        // dNactive/dx is computed above as sumdPAkdxi_noweight.
        // ----------------------------------------------------------------------------------------------------
        ...
        for (int is=Nevents; is<Nevents+Nbatch; is++) {
            ...
            // -------------------------------------------------------------------------------
            for (int iter=0; iter<Ntrials[is]-1 && iter<maxIter; iter++) {
                double r2newx = pow(x[id]+dx-TryX0[is][iter],2)+pow(y[id]-TryY0[is][iter],2);
                double r2newy = pow(x[id]-TryX0[is][iter],2)+pow(y[id]+dy-TryY0[is][iter],2);        
                if (r2newx<=Rslack2) dNx++; 
                if (r2newy<=Rslack2) dNy++; 
                // Now check the other directions, properly accounting for the sign
                // ----------------------------------------------------------------
                r2newx = pow(x[id]-dx-TryX0[is][iter],2)+pow(y[id]-TryY0[is][iter],2);
                r2newy = pow(x[id]-TryX0[is][iter],2)+pow(y[id]-dy-TryY0[is][iter],2);        
                if (r2newx<=Rslack2) dNx--; // a negative dx caused an increase in N 
                if (r2newy<=Rslack2) dNy--; 
            }
            // Now check if accepted shower would fall out of accepted region
            // --------------------------------------------------------------
            bool pass = false;
            ...
            if (!pass) { // that shower was accepted in Nbatch thanks to being close to id only
                // So check if movement would void that
                double r2newx = pow(x[id]+dx-TrueX0[is],2)+pow(y[id]-TrueY0[is],2);
                double r2newy = pow(x[id]-TrueX0[is],2)+pow(y[id]+dy-TrueY0[is],2);        
                if (r2newx>Rslack2) dNx--;
                if (r2newy>Rslack2) dNy--;
                // Also check the other direction, with opposite sign in the effect
                r2newx = pow(x[id]-dx-TrueX0[is],2)+pow(y[id]-TrueY0[is],2);
                r2newy = pow(x[id]-TrueX0[is],2)+pow(y[id]-dy-TrueY0[is],2);        
                if (r2newx>Rslack2) dNx++;
                if (r2newy>Rslack2) dNy++;
            }
        }
        ...
        if (MeasFg!=0. && inv_sigmafs!=0. && N_active!=0. && totNtrials>0) { 
            dU_dxi[id] = U_GF * (d_fs_dx/MeasFg + d_invsigfs_dx/inv_sigmafs 
                                 - 0.5*dNtrials_dx/totNtrials
                                 + dRtot_dx/(aveRdet+Rslack)
                                 //- 0.5*sumdPAkdxi_noweight/N_active
                                 ) + eta_IR*dIR_dxi;
            dU_dyi[id] = U_GF * (d_fs_dy/MeasFg + d_invsigfs_dy/inv_sigmafs 
                                 - 0.5*dNtrials_dy/totNtrials  
                                 + dRtot_dy/(aveRdet+Rslack)
                                 // -0.5*sumdPAkdyi_noweight/N_active
                                 ) + eta_IR*dIR_dxi;
            //cout << "id = " << id << " dudx = " << dU_dxi[id] << " dn/n = " << dNtrials_dx/totNtrials 
            //     << " dR/R = " << dRtot_dx/(aveRdet+Rslack) << " dPa/Pa = " <<  sumdPAkdxi_noweight/N_active << endl;
            //cout << "id = " << id << " dudy = " << dU_dyi[id] << " dn/n = " << dNtrials_dy/totNtrials 
            //     << " dR/R = " << dRtot_dy/(aveRdet+Rslack) << " dPa/Pa = " <<  sumdPAkdyi_noweight/N_active << endl;
        } else {
            cout    << "     Division by zero in dUdx calculation. " << endl;
            outfile << "     Division by zero in dUdx calculation. " << endl;
            warnings2++;
            TerminateAbnormally();
        }
#ifdef PLOTS
...
int main (int argc, char * argv[]) {
    ...
    for (int i=0; i<argc; i++) {
        if (!strcmp(argv[i],"-h")) {
            ...
            cout << "-ete utility weight of integrated energy resolution" << endl;
            ...
            cout << "-esl slope of energy distribution of showers" << endl;
            cout << "-tnu tank number in macroaggregates (for tank area calculation)" << endl;
            cout << "-nsu do NOT do SGD updates" << endl;
            cout << "-itv use true shower parameters for initialization of likelihood reco" << endl;
            return 0;
        }
        ...
        else if (!strcmp(argv[i],"-tnu")) {TankNumber      = atof(argv[++i]);}
        ...
        else if (!strcmp(argv[i],"-nsu")) {doSGDupdate     = false;}    
        else if (!strcmp(argv[i],"-itv")) {initTrueVals    = true;}            
    }  
#endif // STANDALONE
    ...
    // doSGDupdate      = if false, no SGD updates of detector positions is operated (used for U estimates)

#ifdef INROOT
int swgolo (int Nev=1000, int Nba=1000, int Nu=37, int Nep=1000, 
             double Spacing=50., double SpSt = 50., double Rsl=2000., int sh=3, 
             int cm=3, double SLR = 1.0, double Esl = 0., bool OS = true, bool uTA = true, bool uTE = false, int Ntr = 30, int Tnu = 19,
             double etf = 0.1, double ete=10.,int Ngr = 100, int NEi = 10, double LR_E=0.01, double LR_X = 0.5, double LR_A = 0.01) {
    ...
    int Nmultiplets;
    if (CommonMode>=3) {
        if (shape<3 || (shape>4 && shape!=9)) {
            cout << "     Sorry, cannot run with shape different from 3 or 4 if CommonMode>=3. Stopping." << endl;
            return 0;
        } else {
            if (Nunits%CommonMode!=0) {
                cout << "     Sorry, with CommonMode>=3 we need to set Nunits multiple of CommonMode. Rounding it to " << Nunits+CommonMode-Nunits%CommonMode << endl; 
    ...
    outfile << "     doSGDupdate     = " << doSGDupdate << endl;
    ...
    if (readGeom) {
        ...
        DefineLayout (); // -> also defines initial aveRdet
    }

    // Define initial positions (used to fill Rdistr0 histogram while varying aveRdet)
    // -----------------------------------------------------------------------------
    for (int id=0; id<Nunits; id++) {
        xinit[id] = x[id];
        yinit[id] = y[id];
    ...
    // Define number of R bins depending on initial value of aveRdet (which is now defined) and Rslack
    // ---------------------------------------------------------------------------------------------
    NRbins = 100*(aveRdet+Rslack)/aveRdet;
    ...
    TProfile * Uave    = new TProfile ("Uave",     "", Nepochs/10, 0.5, (double)Nepochs+0.5,0.,10000.);  
    TProfile * UaveGF  = new TProfile ("UaveGF",   "", Nepochs/10, 0.5, (double)Nepochs+0.5,0.,10000.);  
    TProfile * UaveIR  = new TProfile ("UaveIR",   "", Nepochs/10, 0.5, (double)Nepochs+0.5,0.,10000.);  
    TH2D * Layout;  
    TH2D * Showers3;
    TH1D * Rdistr0;  // changes at every sgd loop 
    TH1D * Rdistr;
#ifdef FEWPLOTS 
    TH1D * LLRP;
    TH1D * LLRG;
    ...
    do { // SGD
        ...
        // for the generation of showers within a radius Rtot=aveRdet+Rslack and the
        ...
        // ----------------------------------------------------------------------------
        ExposureFactor = (aveRdet+Rslack); // The other factors are picked up later
        ...
        // Now we can redefine plots that depend on aveRdet
        // ----------------------------------------------
        Layout         = new TH2D     ("Layout",   "Layout of array", 500, -aveRdet-Rslack, aveRdet+Rslack, 500, -aveRdet-Rslack, aveRdet+Rslack);
        Showers3       = new TH2D     ("Showers3", "Distribution of shower cores",  200, -aveRdet-Rslack, aveRdet+Rslack, 200, -aveRdet-Rslack, aveRdet+Rslack);
        Rdistr0        = new TH1D     ("Rdistr0",  "R distribution of detectors", NbinsRdistr, 0., aveRdet+Rslack);
        Rdistr         = new TH1D     ("Rdistr",   "R distribution of detectors", NbinsRdistr, 0., aveRdet+Rslack);
        NumStepsvsxy   = new TH2D     ("NumStepsvsxy","",20,0.,aveRdet+Rslack,20,0.1,10.);
        NumStepsvsxyN  = new TH2D     ("NumStepsvsxyN","",20,0.,aveRdet+Rslack,20,0.1,10.);
        ...
        for (int id=0; id<Nunits; id++) {
            ...
            // ------------------------
            Rdistr0->Fill(sqrt(xinit[id]*xinit[id]+yinit[id]*yinit[id])); // as radius expands, histo also changes
        }
        ...
        for (int is=0; is<Nevents+Nbatch; is++) {
            ...
            if (Active[is]) {
                ...
                if (IsGamma[is] && !usetrueE) {
                    // if (epoch==0) DE0->Fill(log(TrueE[is])/log_10,Eerror);
                    // DE->Fill(log(TrueE[is])/log_10,Eerror);
                    if (epoch==0) DE0->Fill(TrueE[is],Eerror);
                    DE->Fill(TrueE[is],Eerror);
                }
            }
#ifdef PLOTS
            if (IsGamma[is]) {
        ...
        for (int k=Nevents; k<Nevents+Nbatch; k++) {
            ...
            if (Pg<epsilon && Pp<epsilon) Nsmallp++;
#ifdef PLOTS
            // Histograms of PDFs:
            // -------------------
            double pgstar = Pg;
        ...
        if (1.*Nsmallp/N_active>0.05) {
            cout    << "     Warning, the number of showers in the pdf generation is insufficient - " << endl;
            cout    << "     " << 100.*Nsmallp/N_active << "% of active showers have negligible estimated pdf. " << endl;
            outfile << "     Warning, the number of showers in the pdf generation is insufficient" << endl;
            outfile << "     " << 100.*Nsmallp/N_active << "% of active showers have negligible estimated pdf. " << endl;
            warnings6++;
        }
        ...
        */
        ...
        // -------------------------------------------------------
        if (CircleExposure) { // Otherwise Ntrials is not filled
            totNtrials = 0;
            for (int is=Nevents; is<Nevents+Nbatch; is++) {
            ...
            // to correct the radius (Area^0.5) by the effective coverage of detectors
            // --------------------------------------------------------------------------------------------------
            if (totNtrials>0.) {
        ...
        UaveGF->Fill(epoch+1,U_GF);
        UaveIR->Fill(epoch+1,U_IR);
        ...
        if (doSGDupdate) {
            ...
            // Compute average du/dx
            // ---------------------
            if (CommonMode==0 || CommonMode>=3) {
                for (int id=0; id<Nunits; id++) {
                    avedUdx += fabs(dU_dxi[id]);
                    avedUdy += fabs(dU_dyi[id]);
                }
                avedUdx = avedUdx/Nunits;
                avedUdy = avedUdy/Nunits;
            }
            ...
            if (CommonMode==0) { // do not vary R; vary independently x and y
                ...
                double kx = 1.;
                if (avedUdx>0.) kx = 0.25*maxDispl/avedUdx;
                double ky = 1.;
                if (avedUdy>0.) ky = 0.25*maxDispl/avedUdy;
                for (int id=0; id<Nunits; id++) {
                    double dx = kx * f * dU_dxi[id];
                    double dy = ky * f * dU_dyi[id];
                    double dr2 = dx*dx+dy*dy;
                    if (dr2>0.) avedr += sqrt(dr2);
                }
                avedr = avedr/Nunits;

                // Cap average movement
                // --------------------
                double multiplier = 1.;
                if (avedr>0.5*maxDispl)  multiplier = 0.5 *maxDispl/avedr; // calibrate movement
                if (avedr<0.05*maxDispl) multiplier = 0.05*maxDispl/avedr; // calibrate movement
                ...
                for (int id=0; id<Nunits; id++) {
                    ...
                    // ----------------------------------------------------------------------------
                    double dx = kx * f * dU_dxi[id] * LearningRate[id] * multiplier;
                    double dy = ky * f * dU_dyi[id] * LearningRate[id] * multiplier;
                    // cout << " id = " << id << " dudx,dudy = " << dU_dxi[id] << "," << dU_dyi[id] 
                    //     << " mult = " << multiplier << " dx,dy = " << dx << "," << dy << endl;

                    // Cap individual movements
                    // ------------------------
                    double dr2 = dx*dx+dy*dy;
                    if (dr2>maxDispl2) {
                        dx  = dx*maxDispl2/dr2;
                    ...
                    x[id]     = x[id] + dx;
                    y[id]     = y[id] + dy;
                    //cout << " id = " << id << "LR = " << LearningRate[id] << " x, dx = " << xprev[id] << " " << dx << " y, yprev = " 
                    //     << yprev[id] << " " << dy << " costh = " << CosThetaEff << endl;
...
#endif
                    double displ2 = dx*dx+dy*dy;
                    if (displ2>0.) aveDispl += sqrt(displ2);
                ...
                for (int id=0; id<Nunits; id++) {
                    ...
                    if (d2>0.) {
                        ...
                        ir = (int)(Ri/(aveRdet+Rslack)*NRbins);
                ...
                // ------------------------------------
                double BoostFactor = 50000.; // experiment with changing LR of this
                for (int ir=0; ir<NRbins; ir++) {
                    if (Ndispl[ir]>0) displ[ir] = BoostFactor*displ[ir]/Ndispl[ir];
                ...
                for (int ir=0; ir<NRbins; ir++) {
                    if (displ[ir]*prev_displ[ir]>0.) {
                        LearningRateR[ir] *= exp(Momentum_coeff); // This will apply to next iteration
                    } else {
                        LearningRateR[ir] *= exp(-Momentum_coeff);
                    }
                }
                ...
                for (int id=0; id<Nunits; id++) {            
                    ...
                    if (d2>0.) {
                        double R = sqrt(d2);
                        int ir = (int)(R/(aveRdet+Rslack)*NRbins);
                    ...
                    if (Rfinal>aveRdet+Rslack) {
                        x[id] = (aveRdet+Rslack-epsilon)*costh;
                        y[id] = (aveRdet+Rslack-epsilon)*sinth;
                    }
                    d2 = pow(dx,2)+pow(dy,2);
                    if (d2>0.) aveDispl += sqrt(d2);     
                    if (debug) cout << "aveDispl " << aveDispl  << " dx,dy " << dx << " " << dy << endl;
                } // end i loop on dets
                ...
                aveDispl = sqrt(pow(commondx,2)+pow(commondy,2))*Nunits;
                for (int id=0; id<Nunits; id++) {
                    x[id] += commondx;
                    y[id] += commondy;
                    if (x[id]>=aveRdet+Rslack)  x[id] = aveRdet+Rslack-epsilon;
                    if (x[id]<=-aveRdet-Rslack) x[id] = -aveRdet-Rslack+epsilon;
                    if (y[id]>=aveRdet+Rslack)  y[id] = aveRdet+Rslack-epsilon;
                    if (y[id]<=-aveRdet-Rslack) y[id] = -aveRdet-Rslack+epsilon;
                }
            } else if (CommonMode>=3) {

                // Detector array is divided in sets of three which share a
                // n-symmetry and whose displacements are the result
                // of averaging the three contributions
                // Beware - below we ASSUME the triplets have same r, and phi
                ...
                avedr = avedr/Nunits;
                // Cap the average displacement regardless of LR
                // ---------------------------------------------
                double multiplier = 1;
                if (avedr>0.5 *maxDispl) multiplier = 0.5 *maxDispl/avedr; // calibrate movement
                if (avedr<0.05*maxDispl) multiplier = 0.05*maxDispl/avedr; // calibrate movement

                for (int id=0; id<Nmultiplets; id++) {

                    // Compute the starting angle of the first element of a multiplet
                    // --------------------------------------------------------------
                    double phifirst = PhiFromXY (x[id],y[id]);
                    ...
                    for (int itr=0; itr<multiplicity; itr++) {
                        int ind = id+itr*Nmultiplets;
                        dx = kx * f * dU_dxi[ind] * LearningRate[id] * multiplier;
                        dy = ky * f * dU_dyi[ind] * LearningRate[id] * multiplier;

                        // Cap each at maxDispl
                        // --------------------
                        double dr2 = dx*dx+dy*dy;
                        if (dr2>maxDispl2) {
                            dx = dx*maxDispl2/dr2;
                        ...
                        // -------------------------------------------------
                        double oldR = pow(x[ind],2)+pow(y[ind],2);
                        if (oldR>0.) {
                            oldR = sqrt(oldR);
                        } else { 
                            oldR = 0.;
                        }
                        double newR = pow(x[ind]+dx,2)+pow(y[ind]+dy,2);
                        if (newR>0.) {
                            newR = sqrt(newR);
                        } else {
                            newR = 0.;
                        ...
                        // ---------------------------------------------------------------------------
                        double oldPhi = PhiFromXY (x[ind],y[ind]);       // 0:2pi
                        double newPhi = PhiFromXY (x[ind]+dx,y[ind]+dy); // 0:2pi
                        double incr   = newPhi-oldPhi;
                        if (incr>pi) {
                            incr = 2.*pi-incr;
                        } else if (incr<-pi) {
                            incr = incr + 2.*pi;
                    ...
                    avedr = avedr/multiplicity;
                    avedp = avedp/multiplicity;
                    double r = pow(x[id],2)+pow(y[id],2);
                    if (r>0.) {
                        r = sqrt(r);
                    } else {
                        r = 0.;
                    ...
                    // -----------------------------------------------------------------------
                    dx = (r+avedr)*cos(phifirst+avedp) - x[id];
                    dy = (r+avedr)*sin(phifirst+avedp) - y[id];
                    double displ2 = dx*dx+dy*dy;
                    if (displ2>0.) {
                        aveDispl += sqrt(displ2);
                        //cout << " id = " << id << " phifirst = " << phifirst << " avedp = " << avedp << " dx,dy = " << (r+avedr)*cos(phifirst+avedp) << "-" << x[id] << " " << (r+avedr)*sin(phifirst+avedp) << "-" << y[id] << " displ = " << sqrt(displ2) << endl;
                    }

                    // Ok, now update the positions of all members of a multiplet
                    // ----------------------------------------------------------
                    for (int itr=0; itr<multiplicity; itr++) {
                        xprev[id+itr*Nmultiplets]   = x[id+itr*Nmultiplets];
                        yprev[id+itr*Nmultiplets]   = y[id+itr*Nmultiplets];
                        x[id+itr*Nmultiplets]       = (r+avedr)*cos(phifirst+avedp+2.*pi*itr/multiplicity);
                        y[id+itr*Nmultiplets]       = (r+avedr)*sin(phifirst+avedp+2.*pi*itr/multiplicity);
                    }
                ...
                aveDispl *= 3; // as later we divide by Nunits
            } // end CommonMode if block
        } // end if doSGDupdate
        // Comment through to here if you want to bypass SGD (for fixed config runs): */

        cout    << "     Epoch = " << epoch 
                << "  Utility value = " << Utility <<  " aveDispl = " << aveDispl/Nunits;
        outfile << "     Epoch = " << epoch 
                << "  Utility value = " << Utility << " aveDispl = " << aveDispl/Nunits;
        if (CommonMode==0) {
            cout    << " avedU = " << avedUdx << "," << avedUdy << " Exposure = " << ExposureFactor;
            outfile << " avedU = " << avedUdx << "," << avedUdy << " Exposure = " << ExposureFactor;
        } else if (CommonMode==1 || CommonMode==2) {
        ...
        // Summary plot
        // ------------
        CT = new TCanvas ("CT","",1400,1100);
        CT->Divide(4,3);
        CT->cd(1);
        if (epoch>0) U->Fit("pol1","Q");
        U->Draw("P");
        Uave->Draw("SAME");
        if (eta_GF>0.) UaveGF->Draw("SAME");
        if (eta_IR>0.) UaveIR->Draw("SAME");
        CT->cd(2);
        Showers3->Draw("COL4");
        Layout->Draw("PSAME");
#ifdef FEWPLOTS
        ...
        if (checkUtility && epoch>0) {
            do {
                ...
                inv_sigmafs  = 1./MeasFgErr; // above we were computing the variance with RCF bound
                cout << endl;
                ComputeUtilityGF(); // computes U_GF
                ComputeUtilityIR(); // computes U_IR
                Utility = U_GF + U_IR;
    ...
    if (!doSGDupdate) {
        ...
        cout    << "       U    = " << sumUGF+sumUIR << " +- " << sqrt(rmsmGF*rmsmGF+rmsmIR*rmsmIR) << endl;
        cout    << endl;
        outfile << endl;
        outfile << "     Average utility after " << Nepochs << " epochs: " << endl;
        outfile << "       U_GF = " << sumUGF << " +- " << rmsmGF << endl;
        outfile << "       U_IR = " << sumUIR << " +- " << rmsmIR << endl;
        outfile << "       U    = " << sumUGF+sumUIR << " +- " << sqrt(rmsmGF*rmsmGF+rmsmIR*rmsmIR) << endl;
        outfile << endl;
    } else {
        ...
        cout << "     U_GF = " << UaveGF->GetBinContent(1) << " +- " << UaveGF->GetBinError(1) << endl;
        cout << "     U_IR = " << UaveIR->GetBinContent(1) << " +- " << UaveIR->GetBinError(1) << endl;
        cout << "     U    = " << Uave->GetBinContent(1)   << " +- " << Uave->GetBinError(1) << endl;
        cout << "     Final utility:" << endl;
        cout << "     U_GF = " << UaveGF->GetBinContent(Nepochs/10) << " +- " << UaveGF->GetBinError(Nepochs/10) << endl;
        cout << "     U_IR = " << UaveIR->GetBinContent(Nepochs/10) << " +- " << UaveIR->GetBinError(Nepochs/10) << endl;
        cout << "     U    = " << Uave->GetBinContent(Nepochs/10)   << " +- " << Uave->GetBinError(Nepochs/10) << endl;
        cout << endl;
        outfile << endl;
        outfile << "     Initial utility:" << endl;
        outfile << "     U_GF = " << UaveGF->GetBinContent(1) << " +- " << UaveGF->GetBinError(1) << endl;
        outfile << "     U_IR = " << UaveIR->GetBinContent(1) << " +- " << UaveIR->GetBinError(1) << endl;
        outfile << "     U    = " << Uave->GetBinContent(1)   << " +- " << Uave->GetBinError(1) << endl;
        outfile << "     Final utility:" << endl;
        outfile << "     U_GF = " << UaveGF->GetBinContent(Nepochs/10) << " +- " << UaveGF->GetBinError(Nepochs/10) << endl;
        outfile << "     U_IR = " << UaveIR->GetBinContent(Nepochs/10) << " +- " << UaveIR->GetBinError(Nepochs/10) << endl;
        outfile << "     U    = " << Uave->GetBinContent(Nepochs/10)   << " +- " << Uave->GetBinError(Nepochs/10) << endl;
        outfile << endl;
    }
...
/*    if (checkmodel) {
    ...
    // -------------------------------------------
    if (writeGeom) SaveLayout(); // HACK try to see if it crashes the program from here
...
// ------------------------------------------------------------------------------------
void CheckProb (int Ntrigger=10, int Nev=1000, int N_trials=1000, int Nu=100, int sh=3,
